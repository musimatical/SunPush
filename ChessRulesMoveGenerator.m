...
class ChessRules:
    ...
    def GetListOfValidMoves(self,chessboard,fromTuple,skipCheckTest=False):
          ...
          return (chessboard.ToTuple(move[1]) for move in self.MoveGenerator(newboard,p,i,chessboard.lastdir,chessboard.recentsquares,skipCheckTest))

    def MoveGenerator(self,newboard,p,i,lastdir,recentsquares,skipCheckTest):
          ...
          for d in directions[p]:
              for j in count(i+d, d):
                  ...
                  # No 'pushbacks'
                  if d == -lastdir and self.ToTuple(i) in recentsquares: break
                  # Move it
                  if q.islower():
                      pieces=[p,q]
                      squares=[i,j]
