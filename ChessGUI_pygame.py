			...
			if not fromSquareChosen and not toSquareChosen:
				...
				if squareClicked != []:
					...
					if currentColor == 'black' and 'b' in squares[r][c]:
						chessboard.Rotate()
						if len(self.Rules.GetListOfValidMoves(chessboard,squareClicked))>0:
							fromSquareChosen = 1
				...
				possibleDestinations = self.Rules.GetListOfValidMoves(chessboard,fromTuple)
				...
				if squareClicked != []:
					...
					if squareClicked in possibleDestinations:
						...
						if squareClicked == fromTuple:
							fromSquareChosen = 0
						elif len(self.Rules.GetListOfValidMoves(chessboard,squareClicked))>0:
							fromSquareChosen = 1
							fromTuple = squareClicked
						else:
